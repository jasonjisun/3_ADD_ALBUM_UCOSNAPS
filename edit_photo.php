...
if (isset($_GET['id'])) {
  ...
  try {
    ...
    $currentTitle = $photo['title']; // Fetch the title as well
    $currentPhotoName = $photo['photo_name'];
...
if (isset($_POST['submit'])) {
  ...
  if ($fileName) {
    ...
    if (move_uploaded_file($tempFileName, $folder)) {
      ...
      $updatedFields[] = "File"; // Track file change
    }
  }
  ...
  if (!empty($updatedFields)) {
    ...
    if (in_array('Title', $updatedFields)) {
      $stmt->bindParam(':title', $title);
    }
    if (in_array('File', $updatedFields)) {
    ...
    try {
      ...
      if ($userId) {
        ...
        $logMessage = 'Updated Photo (' . implode(', ', $updatedFields) . ')';
